...
public abstract class Phase extends GLSLParserBaseListener {
  ...
  void omitNodeTokens(SyntaxTree node) {
    editContext.omitNodeTokens(node);
  }

  void replaceNode(ParserRuleContext node, ParseTree newNode) {
    var children = getSiblings(node);
    children.set(children.indexOf(node), newNode);
    omitNodeTokens(node);
  }

  void removeNode(ParserRuleContext node) {
  ...
  void replaceNode(ParserRuleContext node, String newContents) {
  ...
  public void afterWalk(GLSLParser.TranslationUnitContext TUContext) {
    // to be possibly overwritten by the implementing class
  }
}
